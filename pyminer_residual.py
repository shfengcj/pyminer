...
class JLAresiCal(resiCal):
    ...
    def __init__(self, cosModel, DATA_DIR_JLA ):
        ...
        # cosModel is a object of class CosModel
        
        import numpy as np 
        
        self.ogh2 = 2.469e-5  # Omega_gamma h^2
    ...
    def residual(self, p, fjac=None):
        ...
        covMatrix = self.C00 + p[0]*p[0]*self.C11 +p[1]*p[1]*self.C22 + 2.0*p[0]*self.C01 - 2.0*p[1]*self.C02 - 2.0*p[0]*p[1]*self.C12
        ...
        for i in range(self.numData):
            ...
            res[i]  = self.mb[i] - (p[2]-p[0]*self.x1[i] + p[1]*self.color[i] + p[3]*prob) - ( 5.0*np.log10( (1.0+self.zhel[i])* cdz *3.e5/p[4] )+25.0)
            covMatrix[i,i] += self.dmb[i]*self.dmb[i] + (p[0]*self.dx1[i])*(p[0]*self.dx1[i]) + (p[1]*self.dcolor[i])*(p[1]*self.dcolor[i]) + 2.0*p[0]*self.cov_m_s[i] - 2.0*p[1]*self.cov_m_c[i] - 2.0*p[0]*p[1]*self.cov_s_c[i]
